...
// OF BUSINESS PROFITS, BUSINESS uintERRUPTION, LOSS OF BUSINESS INFORMATION, OR ANY 
...
void Radix008A_CS(uint3 thread_id : SV_DispatchThreadID)
{
	...
	uint imod = (uint)thread_id &(istride - 1);
	uint iaddr = (((uint)thread_id - imod) << 3) + imod;
	for (i = 0; i < 8; i++)
		D[i] = g_SrcData[iaddr + i * istride];

	// Math
	FFT_forward_8(D);
	uint p = (uint)thread_id & (istride - pstride);
	float phase = phase_base * (float)p;
	TWIDDLE_8(D, phase);

	// Store the result
	uint omod = (uint)thread_id & (ostride - 1);
	uint oaddr = (((uint)thread_id - omod) << 3) + omod;
